...
- 配置体系基于 `web/WEB-INF/config.txt`，支持自定义 `ConfigLoad` 扩展与多数据源读写分离。
## 目录速览
...
- `web/WEB-INF`：示例 Web 应用配置，包含 `web.xml` 与默认 `config.txt`。
...
- `easy.config`
	- `Config`：启动时加载 `config.txt`，可通过 `CONFIGLOADCLASS` 指定自定义 `ConfigLoad` 实现对属性解密或动态注入。
- `easy.util`
	- `Log`：集中处理日志输出、文件持久化与邮件告警（依赖 `config.txt` 中的 SMTP 配置）。
...
## 配置文件 `web/WEB-INF/config.txt`
- 采用键值对格式，覆盖项目名称、编码设置、数据库连接、日志策略等。
- 通过 `CONFIGLOADCLASS` 可以自定义类（例：`TestCnfLoad`）进行二次处理，例如敏感字段加解密。
- 文档中出现的数据库账号口令仅为示例，请在生产环境中替换并启用密文存储。
...
- 为避免输出敏感信息，建议结合 `ConfigLoad` 扩展实现脱敏。
## 测试
- 当前 `src/test` 为空，可结合 Kotlin `test` 框架补充单元测试。
