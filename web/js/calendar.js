...
function showcalendar(obj, strFormat) {
	if (strFormat == null) {
...
Calendar.prototype.draw = function() {
	...
	mvAry[mvAry.length] = '<div name="calendarForm" style="margin: 0px; width:200px;">';
	mvAry[mvAry.length] = '    <table style="width:200px;" border="0" cellpadding="0" cellspacing="1">';
	mvAry[mvAry.length] = '      <tr>';
	mvAry[mvAry.length] = '        <th align="left" width="1%"><input style="border: 1px solid '+ calendar.colors["input_border"]+ ';background-color:'	+ calendar.colors["input_bg"]
			+ ';width:16px;height:20px;cursor:pointer;" name="prevMonth" type="button" id="prevMonth" value="&lt;" /></th>';
	mvAry[mvAry.length] = '<th align="center" width="98%" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:14px; width:68px"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px; width:68px"></select></th>';
	...
	mvAry[mvAry.length] = '    <table id="calendarTable"  style="border:0px solid #CCCCCC;background-color:#FFFFFF; font-size:12px; width:200px;" border="0" cellpadding="3" cellspacing="1">';
	mvAry[mvAry.length] = '      <tr>';
	for ( var i = 0; i < 7; i++) {
		mvAry[mvAry.length] = '      <th style="font-weight:normal;font-size:12px;background-color:'+ calendar.colors["tr_bg"]
				+ ';color:'
	...
	for ( var i = 0; i < 6; i++) {
		...
		for ( var j = 0; j < 7; j++) {
			if (j == 0) {
				mvAry[mvAry.length] = '  <td style="cursor:pointer;color:'
						+ calendar.colors["sun_word"] + '; font-size:12px;"></td>';
			} else if (j == 6) {
				mvAry[mvAry.length] = '  <td style="cursor:pointer;color:'
						+ calendar.colors["sat_word"] + ';font-size:12px;"></td>';
			} else {
				mvAry[mvAry.length] = '  <td style="cursor:pointer;font-size:12px;"></td>';
			}
		}
		mvAry[mvAry.length] = '    </tr>';
	}
...
document.write('background-color: #FFFFFF;border: 1px solid #ccc;width:175px;font-size:12px;"></div>');
if (document.all) {
